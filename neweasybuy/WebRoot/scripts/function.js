...
$(function() {
			...
			.click(
					...
					function() {
						...
						if (val == "添加") {
							...
							// 通过ajax动态增加地址
							$.post("addAdrServlet", data, function(result) {
								$("#span").append(
										'<br/><input name="address" type="radio" value="'+addr+'" id="address0"/><span>'
												+ addr + '</span>');
							});
						} else {
							var span = $("#span")
	...
	// 订单号验证，只能为数字
	/*
	 * $("#orderForm").submit(function(){ var flag =
	 * !isNaN($("#entityId").val()); if(!flag){ alert("只能是数字！"); } return flag;
	 * });
	 */
	// 出现大图标
