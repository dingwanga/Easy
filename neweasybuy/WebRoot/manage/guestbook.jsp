...
<body>
	<script type="text/javascript">
		function delComment(ecId) {
			if (confirm("是否要删除留言信息？")) {
				$.post("commentDelServlet", "ecId=" + ecId, callBack);
		...
		<div class="manage">
			<table class="list">
				...
				<tr>
					...
					<c:if test="${not empty comment.ecReply}">
					<td class="w1 c">已回复</td>
					</c:if>
					<c:if test="${empty comment.ecReply}">
					<td class="w1 c">未回复</td>
					</c:if>
					<td class="w1 c"><a href="commentModifyServlet?id=${comment.ecId}">修改</a> <a class="manageDel" href="javascript:delComment(${comment.ecId})">删除</a></td>
				</tr>
				</c:forEach>
			</table>
			<div class="pager">
				<ul class="clearfix">
					<li>当前页/总页数: ${pageComment.currPageNo} /${pageComment.totalPageCount}</li>
				<c:if test="${pageComment.currPageNo>1}">
					<li><a href="commentServlet?pageNo=1">首页</a>
					</li>
					<li><a href="commentServlet?pageNo=${pageComment.currPageNo-1 }">上一页</a>
					</li>
				</c:if>
				<c:if test="${pageComment.currPageNo<pageComment.totalPageCount}">
					<li><a href="commentServlet?pageNo=${pageComment.currPageNo+1 }">下一页</a>
					</li>
					<li><a href="commentServlet?pageNo=${pageComment.totalPageCount}">末页</a>
					</li>
				</c:if>
				</ul>
			</div>
