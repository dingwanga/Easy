...
<script type="text/javascript">
		function delPro(epId) {
			if (confirm("是否要删除商品信息？")) {
				$.post("proDelServlet", "epId=" + epId, callBack);
	...
	<div id="main" class="wrap">
					...
					<c:forEach items="${pbProduct.pageList}" var="product">
						<tr>
							...
							<td class="thumb"><img src="../files/${product.epFileName}" /><a
								href="../product-view.html" target="_self">
									${product.epName}</a>
							</td>
							<td class="w1 c"><a href="productbServlet?id=${product.epId}">修改</a> <a
								class="manageDel" href="javascript:delPro(${product.epId})">删除</a>
							</td>
						</tr>
					</c:forEach>
				</table>
		...
		<div class="pager">
			<ul class="clearfix">
				<li>当前页/总页数: ${pbProduct.currPageNo}
					/${pbProduct.totalPageCount}</li>
				<c:if test="${pbProduct.currPageNo>1}">
					<li><a href="productServlet?pageNo=1">首页</a></li>
					<li><a href="productServlet?pageNo=${pbProduct.currPageNo-1 }">上一页</a>
					</li>
				</c:if>
				<c:if test="${pbProduct.currPageNo<pbProduct.totalPageCount}">
					<li><a href="productServlet?pageNo=${pbProduct.currPageNo+1 }">下一页</a>
					</li>
					<li><a
						href="productServlet?pageNo=${pbProduct.totalPageCount}">末页</a></li>
				</c:if>
			</ul>
		</div>
	</div>
